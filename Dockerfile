...
RUN git clone git@gitcode.com:server/client.git /opt/deploy/bidangjia
RUN cd /opt/deploy/bidangjia \
    && composer install \
...
VOLUME ["/opt/deploy/bidangjia", "/usr/local/var/log/php7", "/var/run/"]
WORKDIR /opt/deploy/bidangjia
